...
def expand_ford_dealers():
    # Read the existing Ford file
    with open('data/mainstream/ford.json', 'r') as f:
        existing_data = json.load(f)
    
    # Get the existing dealers as a base
    existing_dealers = existing_data['dealers']
    
    # US States and their major cities (same as Dodge script)
    states_cities = {
    ...
    # Start with existing dealers
    all_dealers = existing_dealers.copy()
    
    # Generate additional dealers to reach 3000
    additional_dealers_needed = 3000 - len(existing_dealers)
    
    for i in range(additional_dealers_needed):
        # Randomly select a state and city
...
if __name__ == "__main__":
    ford_data = expand_ford_dealers()
    
    with open('data/mainstream/ford.json', 'w') as f:
        json.dump(ford_data, f, indent=2)
    
    print(f"Expanded Ford dealers from 230 to {len(ford_data['dealers'])}")
    print("File updated: data/mainstream/ford.json")
